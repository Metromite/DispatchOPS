...
import { saveWorkbookToDispatchFolder,saveWorkbookWithOfflineAttachments } from './desktopExport';
...
export async function exportPriceChangeExcel(campaigns:Campaign[],items:Item[],assignments:Assignment[],visits:Visit[],drivers:any[]=[]){
  ...
  for(const v of visits){
    ...
    const makeRow=(vi:any)=>({...base,'Photo Link':photo,'Signature Link':sig,'Item':itemMap.get(vi?.item_id)?.item_description||vi?.item_id||'','New Price':vi?itemMap.get(vi.item_id)?.new_public_price??itemMap.get(vi.item_id)?.new_pharmacy_price??'':'','Quantity':vi?.quantity??''});
    if(lineItems.length) lineItems.forEach(vi=>rows.push(makeRow(vi))); else rows.push(makeRow(null));
  }
  const headers=['Price Change List','Driver Code','Driver Name','Date','Time','Customer / Pharmacy','Location','Pharmacist Name','Telephone / Mobile','Item','New Price','Quantity','Status','Notes','Photo Link','Signature Link','Assignment Location'];
  const ws=XLSX.utils.json_to_sheet(rows,{header:headers});
  ws['!cols']=headers.map(h=>({wch:Math.max(12,Math.min(34,h.length+4))}));
  const photoCol=headers.indexOf('Photo Link'), sigCol=headers.indexOf('Signature Link');
